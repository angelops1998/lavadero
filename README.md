...
- **admin**: todo lo del encargado + gestión de **servicios/precios** y de **usuarios** (`/admin`).
- **encargado**: pedidos y clientes.
...
app/
  ...
  models/            user, cliente, servicio, orden (+ items y estados)
  routers/           main, auth, consulta (público), ordenes, clientes, servicios, admin
  templates/         Jinja2 (tema claro, mobile-first)
  static/            css/main.css, js/orden-form.js (suma en vivo)
scripts/             crear_admin.py, seed_servicios.py
```
